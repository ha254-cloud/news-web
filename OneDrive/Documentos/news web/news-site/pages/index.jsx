...
export default function Home() {
  ...
  const stats = await axios.get(`${API_BASE}/api/rewritten-news`, {
  ...
  async function fetchNews() {
    ...
    try {
      ...
      let res = await axios.get(`${API_BASE}/api/rewritten-news`, {
      ...
      if (data.length === 0) {
        ...
        res = await axios.get(`${API_BASE}/api/rewritten-news`, {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Tiny Filter Bar at Top */}
      <div className="w-full bg-white shadow-sm border-b py-2 px-4 flex flex-wrap gap-2 items-center justify-center" style={{position: 'sticky', top: 0, zIndex: 50}}>
        <input 
          value={q} 
          onChange={e=>setQ(e.target.value)} 
          placeholder="Search news..." 
          className="p-1 border rounded-lg min-w-[120px] focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm" 
        />
        <select 
          value={country} 
          onChange={e=>setCountry(e.target.value)} 
          className="p-1 border rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm"
        >
        ...
        <button 
          onClick={()=>setPage(1)} 
          className="px-3 py-1 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors font-medium text-sm"
        >
          Search
        </button>
      </div>
